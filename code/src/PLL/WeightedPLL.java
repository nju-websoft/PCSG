...
public class WeightedPLL {
	...
	void Prune(WeightedGraph w1) throws IOException {						
		...
		//label generate
		int cnt = 0;
//		PrintWriter pw = new PrintWriter("D:\\data\\label.txt");
		for (int i = 0; i < nodeNum; i++) {
			PrunedBfs(oldName[i]);
...
//				pw.println(i + " " + label.get(oldName[i]).size() );
...
//		pw.close();
	...
	public void pllIndexDeal2(WeightedGraph ww, String baseFolder) throws IOException {
		og1 = new OrderGen();	//new order generate
		newName = new int[ww.nodeNum];
		oldName = new int[ww.nodeNum];
		//start chl / pll
		labelSize  = 0;
		//og1.orderGen2(ww, newName, oldName); // mode: CHL, seems needn't to change
		og1.orderGen1(ww, newName, oldName); // mode: PLL
//		System.out.println("NodeNum: " + ww.nodeNum);
		Prune(ww);
//		System.out.println("avg CHL: "+(((double)labelSize)/ww.nodeNum));
		System.out.println("avg PLL: "+(((double)labelSize)/ww.nodeNum));
		orderOutput2(baseFolder, ww);
		labelOutput2(baseFolder, ww);
	}
	...
	void orderOutput2(String baseFolder, WeightedGraph w1) {
		try {
			String subNameFile = baseFolder + "/subName.txt";
			PrintWriter subNameWriter = new PrintWriter(subNameFile);
			for (int i = 0; i < nodeNum; i++) {
				subNameWriter.println(w1.reHashTable.get(oldName[i]));
			}
			subNameWriter.close();
			System.out.println("Sub Name Generate end!");
			HashMap<String, ArrayList<Integer>> oldTable = new HashMap<>();
			String invertedTableFile = baseFolder + "/invertedTable.txt";
			BufferedReader reader = new BufferedReader(new FileReader(invertedTableFile));
			String line = null;
			while ((line = reader.readLine()) != null) {
				String[] token = line.split(":");
				ArrayList<Integer> values = new ArrayList<>();
				for (String iter: token[1].split(" ")) {
					values.add(Integer.parseInt(iter));
				}
				oldTable.put(token[0], values);
			}
			reader.close();
			PrintWriter invTableWriter = new PrintWriter(invertedTableFile);
			for (Map.Entry<String, ArrayList<Integer>> iter: oldTable.entrySet()) {
				String key = iter.getKey();
				for (int val: iter.getValue()) {
					invTableWriter.println(key + ":" + newName[val]);
				}
			}
	...
	void labelOutput2(String baseFolder, WeightedGraph w1) {
		try {
			String hubLabelFile = baseFolder + "/hubLabel.txt";
			File file = new File(hubLabelFile);
			if (!file.exists()) {
				file.createNewFile();
			}
			PrintWriter hubLabelWriter = new PrintWriter(hubLabelFile);
			for (int i = 0; i < nodeNum; i++) {
				int u = oldName[i];
				for (int j = 0; j < label.get(u).size(); j++) {
					EHop ehv = label.get(u).get(j);
//					d1.insertHubLabel(i, ehv.v, ehv.dis, newName[ehv.par]);
					hubLabelWriter.println(i + "\t" + ehv.v + "\t" + ehv.dis + "\t" + newName[ehv.par]);
				}
			}
			hubLabelWriter.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println("hub Label Generate end!");
