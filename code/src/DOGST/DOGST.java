...
public class DOGST {
	...
	public void InitByCommonStruct(CommonStruct c1) {
		com1 = c1;
		hopTerminal = new ArrayList<hopage>();
		epoch = 0;
		for (int i = 0; i < c1.nodeNum; i++) hopTerminal.add(new hopage(epoch));
...
//		int cnt = 0;
//		for (int i = 0; i < keyNum; i++)
//			if (com1.HashKeywordTable.containsKey(keywordList.get(i))) {
//				cnt ++;
//			}
//		System.out.println(cnt +  " " + keyNum);
...
//			System.out.println("iteration: " + k);/////////////////////////////////////////////
			...
			if (chins == 2) {
				...
				for (int t = 0; t < terList.size(); t++) {	//choose t as root
					...
					if (at1.weight < totalweight) {
						totalweight = at1.weight;
						bestTree = at1;
...
//							PrintWriter writer = new PrintWriter("C:/Users/Desktop/test/" + count + ".txt");
				...
				if (ansList.get(k).weight < totalweight) {
					totalweight = ansList.get(k).weight;
					bestTree = ansList.get(k);
				}
			}
	...
	public AnsTree search(CommonStruct c1, List<String> keys, int chins) {
		totalweight = 0.0;
		com1 = c1;
		keywordList = keys;
		keyNum = keywordList.size();
		ansList = new ArrayList<AnsTree>();
		getdisList(chins);
//		getdisList2(chins); // only dbpedia using this, note to change the resultFile path!
		return bestTree;
	}
}
